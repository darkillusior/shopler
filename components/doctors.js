...
function Doctors({posts,setDoctor,doctor}) {
  ...
  return (
    <>
      ...
      <div className= 'justify-center hover:border hover:border-slate-700 h-16 flex items-center hover:cursor-pointer text-center text-black font-bold text-xl' >Select your Doctor:</div>
      {posts.dr.map((product, index) => (
     <> <div className={doctor.index==index?' justify-center hover:border hover:border-slate-700 h-16 flex items-center hover:cursor-pointer text-center text-slate-900  bg-teal-300 shadow-md font-medium text-lg':' justify-center hover:border hover:border-slate-700 h-16 flex items-center hover:cursor-pointer text-center text-slate-500 font-medium text-lg'} onClick={()=>{setDoctor({index,product})}}>{product.name}</div>
